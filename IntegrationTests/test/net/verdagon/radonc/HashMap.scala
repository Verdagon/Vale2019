...
object HashMap {
  val code =
    """
      ...
      |        capacity,
      |        {(index)
      |          let opt: Opt:HashNode:(K, V) = None:HashNode:(K, V)();
      |          = opt;
      |        }),
      |      0)
      |}
      ...
      |        // do nothing
      |      } else {
      |        let node? = (mut (map.table.(i)) = None:HashNode:(K, V)());
      |        let node = get(node?);
      ...
      |  let startIndex = abs(hash mod table.len());
      |  let index = findEmptyIndexForKey(table, startIndex, node.key);
      |
      |  let opt: Opt:HashNode:(K, V) = Some(node);
      |  mut (table.(index)) = opt;
      |}
      |
      ...
      |  let i = 0;
      |  while {i < table.len()} {
      |    let index = (startIndex + i) mod table.len();
      |    let something = table.(index);
      |    if {something.empty?()} {
      |      ret index;
      |    }
      ...
      |  let startIndex = abs(hash mod this.table.len());
      |  let index? = findIndexOfKey(this.table, this.equator, startIndex, key);
      |  if {index?.empty?()} {
      |    let opt: Opt:&V = None:&V();
      |    ret opt;
      |  }
      |  let node = this.table.(index?.get()).get();
      |  let opt: Opt:&V = Some:&V(node.value);
      |  ret opt;
      |}
      |
