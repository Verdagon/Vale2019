...
import net.verdagon.radonc.templar.templata.{FunctionBanner2, Parameter2}
import net.verdagon.radonc.templar.types.{Coord, InterfaceRef2, Own}
import net.verdagon.radonc.vivem.{Heap, IntV, StructInstanceV}
  ...
  test("Map function") {
    val compile = new Compilation(
      """
        ...
        |fn main() {
        |  let a: Opt:Int = Some(9);
        |  let f = MyEquals9Functor();
        |  let b: Opt:Bool = a.map:(Int, Bool)(&f);
        |  = b.getOr:Bool(false);
        |}
        |""".stripMargin)
