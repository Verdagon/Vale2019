...
class ExpressionTests extends FunSuite with Matchers {
  ...
  test("2") {
    compile("4 + 5") shouldEqual FunctionCallPE(LookupPE("+", List()), List(IntLiteralPE(4), IntLiteralPE(5)),true);
  }
  ...
  test("5") {
    compile("x(y)") shouldEqual FunctionCallPE(LookupPE("x", List()), List(LookupPE("y", List())),true);
  }

  test("6") {
    compile("not y") shouldEqual FunctionCallPE(LookupPE("not", List()), List(LookupPE("y", List())),true);
  }

  test("Lending result of function call") {
    compile("&Muta()") shouldEqual LendPE(FunctionCallPE(LookupPE("Muta", List()), List(),true));
  }

  test("Method call") {
    compile("x.shout()") shouldEqual
      FunctionCallPE(DotPE(LookupPE("x",List()),LookupPE("shout",List()),true),List(),true)
  }

  test("Moving method call") {
    compile("x^.shout()") shouldEqual
      FunctionCallPE(DotPE(LookupPE("x",List()),LookupPE("shout",List()),false),List(),true)
  }
  ...
  test("Templated function call") {
    compile("toArray<imm>(&result)") shouldEqual
      FunctionCallPE(
        ...
        List(LendPE(LookupPE("result",List()))),
        true)
  }
    ...
    compile("not y florgle not x") shouldEqual
        FunctionCallPE(
          ...
          List(
            FunctionCallPE(
              LookupPE("not", List()),
              List(LookupPE("y", List())),
              true),
            FunctionCallPE(
              LookupPE("not", List()),
              List(LookupPE("x", List())),
              true)),
          true)
  }
    ...
    compile("a + b florgle x * y") shouldEqual
        FunctionCallPE(
          ...
          List(
            FunctionCallPE(
              LookupPE("+", List()),
              List(LookupPE("a", List()), LookupPE("b", List())),
              true),
            FunctionCallPE(
              LookupPE("*", List()),
              List(LookupPE("x", List()), LookupPE("y", List())),
              true)),
          true)
  }
  ...
  test("Template calling") {
    compile("MyNone<Int>()") shouldEqual
      FunctionCallPE(LookupPE("MyNone", List(NameOrRunePT("Int"))),List(), true)
    compile("MySome<MyNone<Int>>()") shouldEqual
      FunctionCallPE(LookupPE("MySome", List(CallPT(NameOrRunePT("MyNone"),List(NameOrRunePT("Int"))))),List(), true)
  }
